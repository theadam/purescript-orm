...
import TableDefinition (class ColumnType, createType)
import Type.Data.Row (RProxy)
import Type.Data.RowList (RLProxy(RLProxy))
import Type.Proxy (Proxy(..))

class Creatable (t :: # Type) where
  columnDefinitions :: RProxy t -> Array (Tuple String String)
...
class CreateCols (rl :: RowList) where
  coldefs :: RLProxy rl -> Array (Tuple String String)

instance singleListCreatable :: (IsSymbol k, ColumnType v i o) => CreateCols (Cons k v Nil) where
  coldefs _ = [Tuple (reflectSymbol (SProxy :: SProxy k)) (createType (Proxy :: Proxy v))]
else instance listCreatable :: (
  IsSymbol k,
  ColumnType v i o,
  CreateCols rest
) => CreateCols (Cons k v rest) where
  coldefs _ = [Tuple (reflectSymbol (SProxy :: SProxy k)) (createType (Proxy :: Proxy v))] <> coldefs (RLProxy :: RLProxy rest)
